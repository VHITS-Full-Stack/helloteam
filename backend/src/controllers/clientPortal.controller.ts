...
export const getClientTimeRecords = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    // Resolve approver names for time records
    const approverUserIds = [...new Set(timeRecords.map(r => r.approvedBy).filter(Boolean) as string[])];
    const approverAdmins = approverUserIds.length > 0
    ...
    for (const a of approverAdmins) {
      approverNameMap.set(a.userId, `${a.firstName} ${a.lastName}`);
    }
      ...
      for (const date of allDates) {
        ...
        for (const session of daySessions) {
          ...
          const isOffShiftSession = (() => {
            ...
            const sessionStartInTz = new Date(session.startTime.toLocaleString('en-US', { timeZone: clientTimezone }));
            const sessionStartMinutes = sessionStartInTz.getHours() * 60 + sessionStartInTz.getMinutes();
            return sessionStartMinutes > schedEndMinutes;
          })();

          // Detect early check-in (clocked in before schedule start)
          const isEarlyCheckIn = (() => {
            if (!daySchedule?.startTime || !session.startTime) return false;
            const [sH, sM] = daySchedule.startTime.split(':').map(Number);
            const schedStartMinutes = sH * 60 + sM;
            const sessionStartInTz = new Date(session.startTime.toLocaleString('en-US', { timeZone: clientTimezone }));
            const sessionStartMinutes = sessionStartInTz.getHours() * 60 + sessionStartInTz.getMinutes();
            return sessionStartMinutes < schedStartMinutes;
          })();
          ...
          if (session.endTime) {
            // Completed session:
            // - Auto-generated OTs are matched by createdAt window (created at clock-out)
            // - OFF_SHIFT auto-generated OTs only match off-shift sessions
            // - Pre-approved OTs (isAutoGenerated=false) are not tied to a session time,
            //   so include them in the first session of the day (via matchedOTIds dedup)
            ...
            sessionOTEntries = dayOTRequests.filter(ot => {
              ...
              // Pre-approved OTs: include in first unmatched session of the day
              if (!ot.isAutoGenerated) return true;
              // Auto-generated OTs should only match off-shift or early check-in sessions
              // (regular sessions within the schedule should not show OT)
              if (ot.isAutoGenerated && !isOffShiftSession && !isEarlyCheckIn) return false;
              // Auto-generated OTs: match by createdAt timestamp window
            ...
            // and always include pre-approved OTs
            const sessionStart = session.startTime.getTime();
            sessionOTEntries = dayOTRequests.filter(ot => {
              if (matchedOTIds.has(ot.id)) return false;
              if (!ot.isAutoGenerated) return true;
          ...
          // Only auto-generated OTs represent actual worked overtime
          // Pre-requested OTs are just approvals — they don't reduce regular hours
          ...
          // Only for off-shift sessions — regular sessions should not get OT assigned.
          if (sessionOvertimeMinutes === 0 && timeRecord && !isActive && isOffShiftSession) {
          ...
          if (scheduledDurationMins > 0 && sessionMins > scheduledWorkMins) {
            ...
            sessionRegularMinutes = Math.max(0, effectiveSessionMinutes - sessionOvertimeMinutes);
          }
          ...
          empData.records.push({
            ...
            approvedByName: timeRecord?.approvedBy ? (approverNameMap.get(timeRecord.approvedBy) || 'Admin') : null,
            overtimeEntries: (() => {
              ...
              // For active extra-time sessions (clocked in after shift end), create a synthetic entry
              // so the "OT Without Prior Approval" column shows the in-progress overtime
              if (entries.length === 0 && isActive && daySchedule) {
                const schedEndParts = daySchedule.endTime?.split(':').map(Number);
                if (schedEndParts && schedEndParts.length === 2) {
                  const sessionStartInTz = new Date(session.startTime.toLocaleString('en-US', { timeZone: clientTimezone }));
                  const schedEndDate = new Date(sessionStartInTz);
                  schedEndDate.setHours(schedEndParts[0], schedEndParts[1], 0, 0);
                  if (sessionStartInTz > schedEndDate) {
                    const elapsedMins = Math.max(0, Math.round((Date.now() - session.startTime.getTime()) / 60000));
                    entries.push({
                      id: `synth-active-extra-${session.id}`,
                      type: 'OFF_SHIFT' as any,
                      requestedMinutes: elapsedMins,
                      requestedStartTime: null as any,
                      requestedEndTime: null as any,
                      estimatedEndTime: null as any,
                      status: 'PENDING',
                      reason: 'In progress — clocked in after scheduled shift',
                      isAutoGenerated: true,
                      isPreApproved: false,
                    });
                  }
...
export const getClientBilling = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    for (const ce of clientEmployees) {
      const employeeBillingRate = ce.employee.billingRate ? Number(ce.employee.billingRate) : null;
