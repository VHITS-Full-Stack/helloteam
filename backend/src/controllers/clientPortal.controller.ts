    ...
    for (const [empId, emp] of employeeMap) {
      ...
      for (const date of allDates) {
        ...
        for (const session of daySessions) {
          ...
          // Exclude pre-requested OFF_SHIFT OT that is for a future session (requestedStartTime > session end)
          const sessionEndInTz = session.endTime
            ? new Date(new Date(session.endTime).toLocaleString('en-US', { timeZone: clientTimezone }))
            : null;
          ...
          const sessionOvertime = sessionOTEntries
            .filter(ot => {
              ...
              // Pre-requested OFF_SHIFT OT for after this session: don't count as this session's overtime
