...
export const getAdminTimeRecords = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    for (const session of filteredSessions) {
      ...
      // Compute scheduled duration to cap regular hours at schedule time
      let scheduledDurationMins = 0;
      if (session.scheduledStartTime && session.scheduledEndTime) {
        const [sH, sM] = session.scheduledStartTime.split(':').map(Number);
      ...
      // Regular hours: if worked beyond schedule, cap at scheduled work time; otherwise use work - OT
      const scheduledWorkMins = Math.max(0, scheduledDurationMins - breakMins);
      let regularHoursCalc: number;
      if (scheduledDurationMins > 0 && workMinutes > scheduledWorkMins) {
        regularHoursCalc = scheduledWorkMins;
      } else {
        regularHoursCalc = Math.max(0, workMinutes - overtimeMinutes);
      }
      const regularHours = isActive ? 0 : Math.round(regularHoursCalc / 60 * 100) / 100;
