...
export const getAdminTimeRecords = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    for (const session of filteredSessions) {
      ...
      // Helper: get session clock-in time in minutes (client TZ)
      const sessStartMinInTz = (() => {
        const d = new Date(new Date(session.startTime).toLocaleString('en-US', { timeZone: clientTzForOT }));
        return d.getHours() * 60 + d.getMinutes();
      })();

      // Helper: check if a pre-requested OFF_SHIFT OT matches this session by clock-in proximity
      const preRequestedOTMatchesSession = (ot: any): boolean => {
        ...
        // OT's requested start should be within 15 min of session clock-in
        return Math.abs(otStartMin - sessStartMinInTz) <= 15;
      };
      ...
      if (session.endTime) {
        ...
        sessionOTEntries = sameDayOTs.filter(ot => {
          ...
          // Pre-requested OFF_SHIFT OTs: match by proximity of requestedStartTime to session clock-in
          if (ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated) {
            return preRequestedOTMatchesSession(ot);
          }
        ...
        sessionOTEntries = sameDayOTs.filter(ot => {
          ...
          // Pre-requested OFF_SHIFT: match by proximity
          if (ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated) {
            return preRequestedOTMatchesSession(ot);
          }
      ...
      // Detect off-shift sessions: session started after schedule end, OR has a matched pre-requested OFF_SHIFT OT
      const hasPreRequestedOffShift = sessionOTEntries.some(ot => ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated);
      const isOffShiftSession = hasPreRequestedOffShift || (() => {
