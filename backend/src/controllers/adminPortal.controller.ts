...
export const getAdminTimeRecords = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    for (const session of filteredSessions) {
      ...
      // Check if this is the first session for this employee on this date
      const dayKey = `${session.employeeId}_${dateStr}`;
      const daySessions = sessionsByEmpDate.get(dayKey) || [];
      const isFirstSession = daySessions.length === 0 || daySessions[0].id === session.id;
      ...
      if (session.endTime) {
        ...
        sessionOTEntries = sameDayOTs.filter(ot => {
          ...
          // Pre-requested OTs (not auto-generated): assign to the first session of the day
          // They may have been created before any session started
          if (!ot.isAutoGenerated) {
            return isFirstSession;
          }

          // Auto-generated OTs: match by createdAt
        ...
        sessionOTEntries = sameDayOTs.filter(ot => {
          ...
          // Pre-requested OTs: assign to first session
          if (!ot.isAutoGenerated) {
            return isFirstSession;
          }
      ...
      if (isOffShiftSession) {
        ...
        if (sameDayPreRequestedOT) {
          ...
          // No pre-requested OT: clear the regular schedule for off-shift
