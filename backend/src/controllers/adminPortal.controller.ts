...
export const getAdminTimeRecords = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    for (const session of filteredSessions) {
      ...
      // Helper: check if an OFF_SHIFT OT's requestedStartTime falls within a session's time window
      const otStartFitsSession = (ot: any, sessStartMs: number, sessEndMs: number, tz: string): boolean => {
        if (!ot.requestedStartTime || !/^\d{1,2}:\d{2}$/.test(ot.requestedStartTime)) return false;
        try {
          const [h, m] = ot.requestedStartTime.split(':').map(Number);
          const otStartMin = h * 60 + m;
          const sessStartInTz = new Date(new Date(sessStartMs).toLocaleString('en-US', { timeZone: tz }));
          const sessEndInTz = new Date(new Date(sessEndMs).toLocaleString('en-US', { timeZone: tz }));
          const sessStartMin = sessStartInTz.getHours() * 60 + sessStartInTz.getMinutes();
          const sessEndMin = sessEndInTz.getHours() * 60 + sessEndInTz.getMinutes();
          return otStartMin >= sessStartMin - 15 && otStartMin <= sessEndMin + 15;
        } catch { return false; }
      };

      if (session.endTime) {
        // Completed session — match OTs by createdAt (auto-created at clock-out)
        // Also match pre-requested OFF_SHIFT OTs by requestedStartTime overlapping with session
        const sessionEnd = session.endTime.getTime();
        const sessionStart = session.startTime.getTime();
        const clientTzForOT = clientInfo?.timezone || 'UTC';
        sessionOTEntries = sameDayOTs.filter(ot => {
          if (matchedOTIds.has(ot.id)) return false;

          // Pre-requested OFF_SHIFT OTs: ONLY match by requestedStartTime, not createdAt
          // (they may be created hours before the off-shift session starts)
          if (ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated && ot.requestedStartTime) {
            return otStartFitsSession(ot, sessionStart, sessionEnd, clientTzForOT);
          }

          // All other OTs (auto-generated, SHIFT_EXTENSION): match by createdAt
          if (ot.createdAt) {
            const otCreated = ot.createdAt.getTime();
            if (otCreated >= sessionStart - 2 * 60000 && otCreated <= sessionEnd + 10 * 60000) {
              return true;
            }
          }
          return false;
        ...
        // Active session — match auto-generated OTs by createdAt, pre-requested OFF_SHIFT by requestedStartTime
        const sessionStart = session.startTime.getTime();
        const clientTzForOT = clientInfo?.timezone || 'UTC';
        sessionOTEntries = sameDayOTs.filter(ot => {
          if (matchedOTIds.has(ot.id)) return false;

          // Pre-requested OFF_SHIFT: match by requestedStartTime
          if (ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated && ot.requestedStartTime) {
            const sessStartInTz = new Date(new Date(sessionStart).toLocaleString('en-US', { timeZone: clientTzForOT }));
            const sessStartMin = sessStartInTz.getHours() * 60 + sessStartInTz.getMinutes();
            const [h, m] = ot.requestedStartTime.split(':').map(Number);
            return (h * 60 + m) >= sessStartMin - 15;
          }

          // All other OTs: match by createdAt
          if (ot.createdAt && ot.createdAt.getTime() >= sessionStart - 2 * 60000) {
            return true;
          }
          return false;
        });
      ...
      if (isOffShiftSession) {
        // Prefer pre-requested OT (has the originally requested times) over auto-generated
        const preRequestedOT = sessionOTEntries.find(ot => ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated);
        const autoOT = sessionOTEntries.find(ot => ot.type === 'OFF_SHIFT' && ot.isAutoGenerated);
        const offShiftOT = preRequestedOT || autoOT;

        if (preRequestedOT?.requestedStartTime && preRequestedOT?.requestedEndTime) {
          // Pre-requested OT: show the originally requested time as schedule
          effectiveScheduledStart = preRequestedOT.requestedStartTime;
          effectiveScheduledEnd = preRequestedOT.requestedEndTime;

          // Recalculate late against OT requestedStartTime (HH:MM string)
          const [otH, otM] = preRequestedOT.requestedStartTime.split(':').map(Number);
          ...
          // Auto-generated OT only: no pre-requested schedule, clear the regular schedule
          effectiveScheduledStart = null;
          effectiveScheduledEnd = null;
