    ...
    for (const session of filteredSessions) {
      ...
      if (session.endTime) {
        ...
        sessionOTEntries = sameDayOTs.filter(ot => {
          ...
          // Match by createdAt (auto-generated OTs created near session time)
          if (ot.createdAt) {
          ...
          // Match pre-requested OFF_SHIFT OTs by requestedStartTime falling within session window
          if (ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated && ot.requestedStartTime) {
            try {
              const [h, m] = ot.requestedStartTime.split(':').map(Number);
              const sessionStartInTz = new Date(new Date(sessionStart).toLocaleString('en-US', { timeZone: clientTzForOT }));
              const sessionEndInTz = new Date(new Date(sessionEnd).toLocaleString('en-US', { timeZone: clientTzForOT }));
              const otStartMin = h * 60 + m;
              const sessStartMin = sessionStartInTz.getHours() * 60 + sessionStartInTz.getMinutes();
              const sessEndMin = sessionEndInTz.getHours() * 60 + sessionEndInTz.getMinutes();
              // OT requested start is near (within 15 min) the session window
              if (otStartMin >= sessStartMin - 15 && otStartMin <= sessEndMin + 15) {
                return true;
              }
            } catch { /* skip */ }
          }
        ...
        // Active session — only show OTs created after this session started, or pre-requested OFF_SHIFT matching session
        const sessionStart = session.startTime.getTime();
        const clientTzForOT = clientInfo?.timezone || 'UTC';
        sessionOTEntries = sameDayOTs.filter(ot => {
          if (matchedOTIds.has(ot.id)) return false;
          ...
          // Match pre-requested OFF_SHIFT OTs by requestedStartTime
          if (ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated && ot.requestedStartTime) {
            try {
              const [h, m] = ot.requestedStartTime.split(':').map(Number);
              const sessionStartInTz = new Date(new Date(sessionStart).toLocaleString('en-US', { timeZone: clientTzForOT }));
