...
export const getAdminTimeRecords = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    for (const session of filteredSessions) {
      ...
      // Helper: check if this session is an off-shift session (started after schedule end)
      const isSessionPostShift = (() => {
        if (!session.scheduledEndTime || !session.startTime) return false;
        const [eH, eM] = session.scheduledEndTime.split(':').map(Number);
        const schedEndMin = eH * 60 + eM;
        const tz = clientInfo?.timezone || 'UTC';
        const startInTz = new Date(new Date(session.startTime).toLocaleString('en-US', { timeZone: tz }));
        return (startInTz.getHours() * 60 + startInTz.getMinutes()) > schedEndMin;
      })();
      ...
      if (session.endTime) {
        ...
        sessionOTEntries = sameDayOTs.filter(ot => {
          ...
          // Pre-requested OFF_SHIFT OTs: only match to off-shift sessions (post-shift),
          // never to regular sessions — even if createdAt falls within the regular session window
          if (ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated) {
            return isSessionPostShift;
          }
        ...
        sessionOTEntries = sameDayOTs.filter(ot => {
          ...
          // Pre-requested OFF_SHIFT: only match to off-shift sessions
          if (ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated) {
            return isSessionPostShift;
          }
