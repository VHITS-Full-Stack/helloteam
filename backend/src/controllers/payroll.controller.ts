...
export const getPayrollReadinessDashboard = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const summary = {
      ...
      totalHours: Math.round(clientStats.reduce((sum, c) => sum + c.totalHours, 0) * 10) / 10,
      approvedHours: Math.round(clientStats.reduce((sum, c) => sum + c.approvedHours, 0) * 10) / 10,
      pendingHours: Math.round(clientStats.reduce((sum, c) => sum + c.pendingHours, 0) * 10) / 10,
      overtimeHours: Math.round(clientStats.reduce((sum, c) => sum + c.overtimeHours, 0) * 10) / 10,
  ...
  try {
    ...
    const employees = Object.values(employeeSummary).map((emp: any) => {
      ...
      return {
        ...
        approvedHours: Math.round(emp.approvedMinutes / 60 * 10) / 10,
        pendingHours: Math.round(emp.pendingMinutes / 60 * 10) / 10,
    ...
    res.json({
      ...
      data: {
        ...
        totals: {
          ...
          totalHours: Math.round(employees.reduce((sum, e) => sum + e.totalHours, 0) * 10) / 10,
          overtimeHours: Math.round(employees.reduce((sum, e) => sum + e.overtimeHours, 0) * 10) / 10,
          approvedHours: Math.round(employees.reduce((sum, e) => sum + e.approvedHours, 0) * 10) / 10,
          pendingHours: Math.round(employees.reduce((sum, e) => sum + e.pendingHours, 0) * 10) / 10,
