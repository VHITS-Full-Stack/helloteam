...
export const getSessionHistory = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const sessionsWithStats = sessions.map(session => {
      ...
      // Match OvertimeRequests to this session by date AND time overlap
      const sameDayOTs = overtimeRequests.filter(ot => ot.date.toISOString().split('T')[0] === sessionDateKey);
      const clientTz = clientAssignment?.client?.timezone || 'UTC';
      const sessionStartH = parseInt(new Intl.DateTimeFormat('en-US', { timeZone: clientTz, hour: 'numeric', hourCycle: 'h23' }).format(session.startTime));
      const sessionStartM = parseInt(new Intl.DateTimeFormat('en-US', { timeZone: clientTz, minute: 'numeric' }).format(session.startTime));
      const sessionStartTotal = sessionStartH * 60 + sessionStartM;
      const sessionEndTotal = session.endTime
        ? parseInt(new Intl.DateTimeFormat('en-US', { timeZone: clientTz, hour: 'numeric', hourCycle: 'h23' }).format(session.endTime)) * 60 +
          parseInt(new Intl.DateTimeFormat('en-US', { timeZone: clientTz, minute: 'numeric' }).format(session.endTime))
        : sessionStartTotal;

      const matchedOTEntries: typeof overtimeRequests = sameDayOTs.filter(ot => {
        ...
        if (ot.type === 'OFF_SHIFT' && ot.requestedStartTime) {
          ...
          // Session overlaps with OT window (session started within or near the OT window)
        ...
        if (ot.type === 'SHIFT_EXTENSION' && ot.estimatedEndTime) {
          ...
          // Session should overlap with the extension period (session end is near/past the estimated end)
          return sessionEndTotal >= otEndTotal - (ot.requestedMinutes || 0) - 30 && sessionStartTotal <= otEndTotal + 30;
        }
        // For OTs created from this session (match by createdAt within session time range)
