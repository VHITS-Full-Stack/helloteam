        ...
        clientAssignments.map(async (assignment) => {
          ...
          if (sessionOT > OT_GRACE_MINUTES) {
            try {
              ...
              // Check for existing auto-generated OT request to avoid duplicates
              // Only skip if there's already an auto-generated non-rejected OT of the same type
              await prisma.overtimeRequest.create({
                data: {
                  employeeId: employee.id,
                  clientId: assignment.clientId,
...
export const getSessionHistory = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const sessionsWithStats = sessions.map(session => {
      ...
      // Exclude pre-requested OFF_SHIFT OT whose requestedStartTime is after this session ends
      // (it's for a future off-shift session, not this regular session)
      const sessionEndInTz = session.endTime
        ? new Date(new Date(session.endTime).toLocaleString('en-US', { timeZone: clientTz }))
        : null;
      const sessionEndMin = sessionEndInTz ? sessionEndInTz.getHours() * 60 + sessionEndInTz.getMinutes() : Infinity;
        ...
        ? matchedOTEntries
            .filter(ot => {
              ...
              if (ot.type === 'OFF_SHIFT' && !ot.isAutoGenerated && ot.requestedStartTime) {
                ...
                if ((h * 60 + m) > sessionEndMin) return false;
              }
              return true;
            })
