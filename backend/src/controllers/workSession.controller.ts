...
export const clockOut = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (clientAssignments.length > 0) {
      ...
      console.log(`[ClockOut-Debug] shiftExtensionMinutes=${shiftExtensionMinutes}, overtimeMinutes=${overtimeMinutes}, totalWorkMinutes=${totalWorkMinutes}, earlyOT=${earlyOvertimeMinutes}`);

      // When "Overtime Requires Approval" is UNCHECKED: treat shift extension/late OT as regular time
        ...
        clientAssignments.map(async (assignment) => {
          ...
          if (existing) {
            ...
            const updateData: any = {
              ...
              isLate: isExtraTime ? false : billingUpdate.isLate,
            ...
            await prisma.timeRecord.create({
              data: {
                ...
                isLate: isExtraTime ? false : billing.isLate,
          ...
          if (otRequiresApproval && sessionOT > 0) {
            try {
              const otType = isExtraTime ? 'OFF_SHIFT' : 'SHIFT_EXTENSION';
              // Check for existing OT request (manual or auto-generated) to avoid duplicates
              ...
              if (!existingOT) {
                ...
                console.log(`[OT-Auto] Skipped — existing ${otType} OT request found for employee ${employee.id}, client ${assignment.clientId}, date ${today.toISOString()}`);
              }
            } catch (e) {
    ...
    // --- Send response immediately, do notifications/logging in background ---
    ...
    // --- Background: notifications, logging, IP check (non-blocking) ---
    (async () => {
      try {
        ...
        if (clockInIp && clientIp !== clockInIp) {
          await createSessionLog(
            activeSession.id, userId, employeeName, 'IP_CHANGED',
            `.: IP address changed. Clocked in from ${clockInIp}, clocking out from ${clientIp}`,
            clientIp, { clockInIp, clockOutIp: clientIp }
          );
        }

        await createSessionLog(
          activeSession.id, userId, employeeName, 'CLOCK_OUT',
          `.: Clocked out user: ${user?.email || 'Unknown'}. Shift total ${shiftTotal}`,
          clientIp, { email: user?.email, totalWorkMinutes, shiftTotal }
        );
      } catch (e) {
        console.error('[ClockOut-Background] Error:', e);
      }
    })();
  } catch (error) {
    ...
    const sessionsWithStats = sessions.map(session => {
      ...
      if (session.endTime) {
        ...
        matchedOTEntries = sameDayOTs.filter(ot => {
          ...
          // Pre-requested OTs: wider window (created anytime up to session end)
          if (!ot.isAutoGenerated) {
            return otCreated <= sessionEnd + 10 * 60000;
          }
          // Auto-generated OTs: tight window around session
        ...
        matchedOTEntries = sameDayOTs.filter(ot => {
          ...
          // Auto-generated OTs: tight window
          return ot.createdAt.getTime() >= sessionStart - 2 * 60000;
        });
      }

      // Only count auto-generated OTs as actual overtime (created at clock-out with real work data)
      ...
      const sessionOvertimeMinutes = matchedOTEntries.length > 0
        ? matchedOTEntries
            .filter(ot => ot.status !== 'REJECTED' && ot.isAutoGenerated)
      ...
      return {
        ...
        scheduledStart: session.scheduledStartTime || timeRecord?.scheduledStart || null,
        scheduledEnd: session.scheduledEndTime || timeRecord?.scheduledEnd || null,
