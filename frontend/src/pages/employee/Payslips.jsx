    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center gap-3">
          <button onClick={() => { setSelectedPayslip(null); setDetailData(null); }} className="p-2 hover:bg-gray-100 rounded-lg">
            <ChevronRight className="w-5 h-5 text-gray-500 rotate-180" />
          </button>
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Payslip</h2>
            <p className="text-sm text-gray-500">{d.clientName} &middot; {formatPeriod(d.periodStart, d.periodEnd)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3">
          ...
          <div className="bg-orange-50 rounded-lg p-3 border border-orange-200">
            ...
            <p className="text-xl font-bold text-orange-700">{d.overtimeHours}h</p>
          </div>
          ...
          <div className="rounded-lg p-3 border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm font-bold text-teal-700">+${d.totalBonuses}</p>
              </div>
              <div className="w-px h-5 bg-gray-200"></div>
              <div>
                <p className="text-[10px] text-red-600 font-medium">Deduct</p>
                <p className="text-sm font-bold text-red-700">-${d.totalDeductions}</p>
              </div>
            </div>
          </div>
          <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200">
            <p className="text-[10px] text-emerald-600 font-medium">Gross Pay</p>
            <p className="text-xl font-bold text-emerald-700">${d.grossPay.toLocaleString()}</p>
          </div>
        </div>
        ...
        <Card>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-gray-900">Daily Timesheet</h3>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  ...
                  <th className="text-center px-3 py-2 text-xs font-semibold text-gray-500 uppercase">Billing In/Out</th>
                  <th className="text-center px-3 py-2 text-xs font-semibold text-gray-500 uppercase">Break</th>
                  <th className="text-center px-3 py-2 text-xs font-semibold text-gray-500 uppercase">Regular</th>
                  <th className="text-center px-3 py-2 text-xs font-semibold text-gray-500 uppercase">OT</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {d.dailyRecords.map((rec, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    <td className="px-3 py-2 font-medium text-gray-900">{formatDate(rec.date)}</td>
                      ...
                      {rec.billingStart && rec.billingEnd ? (
                        <span>
                          ...
                          {rec.approvedOTMinutes > 0
                            ? formatTime(new Date(new Date(rec.billingEnd).getTime() + rec.approvedOTMinutes * 60000))
                            : formatTime(rec.billingEnd)}
                        </span>
                      ) : '—'}
                    </td>
                    <td className="px-3 py-2 text-center text-gray-500">{rec.breakMinutes > 0 ? formatHours(rec.breakMinutes) : '—'}</td>
                    <td className="px-3 py-2 text-center font-semibold text-gray-900">{formatHours(rec.regularMinutes)}</td>
                    <td className="px-3 py-2 text-center">
                      {rec.approvedOTMinutes > 0
                        ? <span className="text-orange-600 font-medium">{formatHours(rec.approvedOTMinutes)}</span>
                        : '—'}
                    </td>
                    <td className="px-3 py-2 text-center font-bold text-blue-700">{formatHours(rec.payableMinutes)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-gray-300 bg-gray-50">
                  ...
                  <td></td>
                  <td></td>
                  <td className="px-3 py-2 text-center font-bold text-gray-900">{d.regularHours}h</td>
                  <td className="px-3 py-2 text-center font-bold text-orange-600">{d.overtimeHours}h</td>
                  <td className="px-3 py-2 text-center font-bold text-blue-700">{d.totalHours}h</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <Card>
          ...
          <div className="space-y-1.5">
            ...
            {d.adjustments?.map((adj) => (
              <div key={adj.id} className={`flex items-center gap-3 px-3 py-2 rounded-lg ${adj.type === 'BONUS' ? 'bg-green-50/50' : 'bg-red-50/50'}`}>
                <div className={`w-0.5 h-6 rounded-full ${adj.type === 'BONUS' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                <span className={`text-sm flex-1 ${adj.type === 'BONUS' ? 'text-green-700' : 'text-red-700'}`}>
                  {adj.type === 'BONUS' ? 'Bonus' : 'Deduction'}
                  <span className="text-xs text-gray-400 ml-1">({adj.reason})</span>
                </span>
                <span className={`text-sm font-semibold w-20 text-right ${adj.type === 'BONUS' ? 'text-green-700' : 'text-red-700'}`}>
            ...
            <div className="border-t border-dashed border-gray-300 mt-2 pt-2">
              <div className="flex items-center gap-3 px-3 py-2 bg-emerald-50 rounded-lg">
                ...
                <span className="text-sm font-bold text-gray-900 flex-1">Gross Pay</span>
                <span className="text-lg font-bold text-emerald-700 w-20 text-right">${d.grossPay.toLocaleString()}</span>
              </div>
            </div>
          </div>
        </Card>
      ...
      {payslips.length === 0 ? (
        <Card>
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-400 mt-1">Payslips will appear here after payroll is finalized</p>
          </div>
        </Card>
      ) : (
        <Card padding="none">
