          ...
          {(() => {
            ...
            for (let d = new Date(iterStart); d <= iterEnd; d.setDate(d.getDate() + 1)) {
              ...
              if (sessions.length === 0) {
                // No sessions — skip on client side
              } else {
                ...
                sessions.forEach((rec) => {
                  ...
                  // Skip days where employee didn't work (no clock-in, 0 minutes, not leave/holiday)
                  const clockIn = rec.billingStart || rec.clockIn;
                  if (!isLeaveOrHoliday && !clockIn && totalM === 0) return;
