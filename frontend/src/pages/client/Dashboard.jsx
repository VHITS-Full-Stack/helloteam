...
const ClientDashboard = () => {
  ...
  const fetchDashboardData = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      const [statsRes, activeRes, upcomingOTRes, pendingOTRes, leaveRes, payrollRes] =
        await Promise.allSettled([
          clientPortalService.getDashboardStats(),
          clientPortalService.getActiveEmployees(),
          overtimeService.getOvertimeRequests({ status: 'PENDING', isAutoGenerated: 'false' }),
  ...
  return (
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-amber-800 mt-1">
                Please approve or deny before your billing cycle closes. Unapproved hours cannot be processed for payment.
              </p>
              {pendingOT.employees && pendingOT.employees.length > 0 && (
                <div className="mt-3 space-y-2">
                  {pendingOT.employees.map((emp, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2 bg-white/70 border border-amber-200 rounded-lg">
                      <AlertCircle className="w-4 h-4 text-amber-500 flex-shrink-0" />
                      <span className="text-sm text-amber-900">
                        <strong>{emp.name}</strong> has <strong>{emp.hours}</strong> of unapproved overtime.
                        {" "}Please approve or deny before your account is charged.
                      </span>
                    </div>
                  ))}
                </div>
      ...
      {/* Alerts */}
      {((pendingOT && pendingOT.count > 0) || upcomingOT.length > 0 || pendingLeaveCount > 0) && (
        <Card>
          <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-3">Alerts</h3>
          <div className="space-y-2">
            {pendingOT && pendingOT.count > 0 && (
              ...
              >
                ...
                <span className="text-sm text-gray-800 flex-1">
                  <strong>{pendingOT.count}</strong> unapproved overtime {pendingOT.count === 1 ? 'needs' : 'need'} review
                </span>
            ...
            {upcomingOT.length > 0 && (
              ...
              >
                ...
                <span className="text-sm text-gray-800 flex-1">
                  <strong>{upcomingOT.length}</strong> overtime {upcomingOT.length === 1 ? 'request' : 'requests'} pending your approval
                </span>
                <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-gray-600" />
              </button>
            )}
            {pendingLeaveCount > 0 && (
              ...
              >
                ...
                <span className="text-sm text-gray-800 flex-1">
                  <strong>{pendingLeaveCount}</strong> leave {pendingLeaveCount === 1 ? 'request' : 'requests'} pending your approval
                </span>
                <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-gray-600" />
              </button>
            )}
        ...
        <Card>
          ...
          {upcomingOT.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-1">Pre-approved OT requests will appear here</p>
            </div>
          ) : (
            <div className="space-y-3">
              {upcomingOT.slice(0, 5).map((ot) => {
                const empName = ot.employee ? `${ot.employee.firstName} ${ot.employee.lastName}` : "Unknown";
                const hrs = Math.floor(ot.requestedMinutes / 60);
                const mins = ot.requestedMinutes % 60;
                const duration = mins > 0 ? `${hrs}h ${mins}m` : `${hrs}h`;
                const dateLabel = new Date(ot.date).toLocaleDateString("en-US", {
                  weekday: "short",
                  month: "short",
                  day: "numeric",
                  timeZone: "UTC",
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Avatar name={empName} src={ot.employee?.profilePhoto} size="sm" />
                        <div>
                          <p className="font-medium text-gray-900 text-sm">{empName}</p>
                          <p className="text-xs text-gray-500">{dateLabel}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <Badge variant={ot.type === "OFF_SHIFT" ? "info" : "secondary"} size="sm">
                          {ot.type === "OFF_SHIFT" ? "Off-Shift" : "Extension"}
                        </Badge>
                        <p className="text-sm font-semibold text-gray-900 mt-1">{duration}</p>
                      </div>
                    </div>
                    {ot.reason && (
                      <p className="text-xs text-gray-500 mt-2 line-clamp-1">{ot.reason}</p>
                    )}
                  </div>
                );
              })}
