...
const TimeRecords = () => {
  ...
  const confirmRejectOT = async () => {
    ...
    try {
      setActionLoading(true);
      const response = await clientPortalService.rejectOvertime(rejectOTId, rejectOTReason);
      if (response.success) {
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {emp.filteredRecords.map((rec) => {
                        ...
                        // Pre-requested OT (employee/client requested in advance)
                        const requestedOTEntries = otEntries.filter((ot) => !ot.isAutoGenerated);
                        const approvedRequestedOT = requestedOTEntries.filter((ot) => ot.status === "APPROVED" || ot.status === "AUTO_APPROVED");
                        const pendingRequestedOT = requestedOTEntries.filter((ot) => ot.status === "PENDING");
                        ...
                        return (
                          ...
                          >
                            ...
                            {isLeaveOrHoliday ? (
                              <td colSpan={7} className="py-2.5 px-3 text-center">
                              ...
                              <>
                                <td className="py-2.5 px-3 text-center text-sm">
                                  {clockIn ? (
                                    <span className="text-gray-900">{formatClockTime(clockIn, clientTimezone)}</span>
                                  ) : (
                                ...
                                <td className="py-2.5 px-3 text-center text-sm">
                                  {clockOut ? (
                                    <span className="text-gray-900">{formatClockTime(clockOut, clientTimezone)}</span>
                                  ) : clockIn ? (
                                    <span className="text-green-600 font-medium text-xs">In Progress</span>
                                  ) : (
                                    <span className="text-gray-300">—</span>
                                  )}
                                </td>
                        ...
                        {!isLeaveOrHoliday && (
                          <>
                            <div className="flex items-center gap-4 text-xs text-gray-500">
                              {clockIn && (
                                <span>In: {formatClockTime(clockIn, clientTimezone)}</span>
                              )}
                              {clockOut ? (
                                <span>Out: {formatClockTime(clockOut, clientTimezone)}</span>
                              ) : clockIn ? (
