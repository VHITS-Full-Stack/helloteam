...
const Approvals = () => {
  ...
  const [activeType, setActiveType] = useState(
    searchParams.get('type') === 'overtime' || searchParams.get('tab') === 'overtime' ? 'overtime'
    : searchParams.get('type') === 'auto-overtime' ? 'autoOvertime'
    : 'leave'
  );
  ...
  const fetchingAutoOvertimeRef = useRef(false);
      ...
      (async () => {
        try {
          const [summaryRes, autoRes] = await Promise.all([
            overtimeService.getOvertimeSummary({}),
            overtimeService.getOvertimeRequests({ status: 'PENDING', isAutoGenerated: 'true' }),
          ]);
          ...
          if (autoRes.success) {
            ...
            setAutoOvertimeSummary(prev => ({ ...prev, pending: autoPending }));
            // Auto-switch to auto OT tab if there are pending auto OT
            if (autoPending > 0 && activeType === 'leave' && !searchParams.get('type')) {
  ...
  useEffect(() => {
    ...
    setSelectedItems([]);
  }, [activeType, activeTab, fetchApprovals, fetchOvertimeRequests, fetchAutoOvertimeRequests]);
  ...
  const getTabCount = (tabId) => {
    if (activeType === 'leave') {
      return tabId === 'pending' ? summary.pending : tabId === 'approved' ? summary.approvedThisWeek : summary.rejectedThisWeek;
    }
    if (activeType === 'autoOvertime') {
      return tabId === 'pending' ? autoOvertimeSummary.pending : tabId === 'approved' ? autoOvertimeSummary.approved : autoOvertimeSummary.rejected;
    }
    return tabId === 'pending' ? overtimeSummary.pending : tabId === 'approved' ? overtimeSummary.approved : overtimeSummary.rejected;
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <Card padding="md">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-end gap-2">
              <Button
                variant="primary"
                onClick={() => activeType === 'leave' ? fetchApprovals() : activeType === 'autoOvertime' ? fetchAutoOvertimeRequests() : fetchOvertimeRequests()}
              >
                Apply
              </Button>
              <Button
      ...
      <div className="flex gap-2 p-1 bg-gray-100 rounded-lg w-fit">
        ...
        <button
          onClick={() => { setActiveType('autoOvertime'); setSelectedItems([]); }}
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
            activeType === 'autoOvertime'
              ? 'bg-white text-gray-900 shadow-sm'
              : 'text-gray-600 hover:text-gray-900'
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                {activeType === 'leave' ? 'Overtime Requests' : activeType === 'autoOvertime' ? 'Needs Approval' : 'Pending Hours'}
              </p>
            </div>
          </div>
        </Card>
      ...
      {loading ? (
        ...
        <Card padding="none">
          {(activeType === 'overtime' ? overtimeRequests : autoOvertimeRequests).length > 0 ? (
            <Table>
              <TableHead>
                <TableRow>
                  {activeTab === 'pending' && (
              ...
              <TableBody>
                {(activeType === 'overtime' ? overtimeRequests : autoOvertimeRequests).map((request) => (
        ...
        {selectedItem && (
          <div className="space-y-4">
            <p className="text-gray-600">
              Are you sure you want to approve this {activeType === 'leave' ? 'leave request' : 'overtime request'}?
            </p>
            <div className="p-4 bg-gray-50 rounded-lg space-y-2">
              ...
              {(activeType === 'overtime' || activeType === 'autoOvertime') && selectedItem.type === 'OFF_SHIFT' && selectedItem.requestedStartTime && (
                <div className="flex justify-between">
              ...
              {(activeType === 'overtime' || activeType === 'autoOvertime') && selectedItem.reason && (
                <div className="flex justify-between">
      ...
      <Modal
        ...
        title={(activeType === 'overtime' || activeType === 'autoOvertime') ? 'Deny Overtime' : 'Deny Leave Request'}
        size="sm"
        footer={
          <>
