...
const TimesheetDetail = () => {
  ...
  // Build session records sorted by date
  const sessionRecords = (record?.records || [])
      ...
      {loading ? (
        ...
        <>
          ...
          {(() => {
            ...
            // Build rows from session records
            const dayRows = sessionRecords
              .filter((rec) => {
                const dayOfWeek = rec.dateObj.getUTCDay();
                const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
                const totalM = rec.totalMinutes || 0;
                if (isWeekend && totalM === 0 && rec.status?.toLowerCase() === "not_started") return false;
                return true;
              })
              .map((rec) => {
                ...
                return {
                  key: rec.id || `${date.getTime()}-${clockIn}`,
                  date,
                  rec,
                  isToday,
                  recStatus,
