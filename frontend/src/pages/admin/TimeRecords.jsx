...
const TimeRecords = () => {
  ...
  const handleExportPDF = () => {
    ...
    for (const emp of timeRecords) {
      for (const day of emp.dailyRecords) {
        const otEntries = day.overtimeEntries || [];
        const requestedOTMins = otEntries.filter(o => !o.isAutoGenerated || o.type === 'SHIFT_EXTENSION').reduce((s, o) => s + (o.requestedMinutes || 0), 0);
        const autoOTMins = otEntries.filter(o => o.isAutoGenerated && o.type !== 'SHIFT_EXTENSION').reduce((s, o) => s + (o.requestedMinutes || 0), 0);
        rows.push([
          emp.employee,
          emp.client,
          day.date,
                        ...
                        <td className="px-4 py-3 text-sm">
                          {(() => {
                            const autoOT = (day.overtimeEntries || []).filter(
                              (o) => o.isAutoGenerated && o.type !== 'SHIFT_EXTENSION',
                            );
