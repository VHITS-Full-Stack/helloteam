...
const TimeRecords = () => {
  ...
  const handleDownloadTimesheetPdf = async () => {
    ...
    const filename =
      `timesheet-${clientName}-${pdfStartDate}-to-${pdfEndDate}.pdf`
        .replace(/\s+/g, "-")
        .replace(/[^a-z0-9._-]/gi, "-");
      ...
      <div className="space-y-3">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            >
              <span className="whitespace-nowrap">Download Timesheet PDF</span>
            </Button>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {timeRecords
                  ...
                  .sort((a, b) => {
                    ...
                    switch (sortField) {
                      case "name":
                        cmp = (a.empRecord.employee || "").localeCompare(
                          b.empRecord.employee || "",
                        );
                        break;
                      case "client":
                        cmp = (a.empRecord.client || "").localeCompare(
                          b.empRecord.client || "",
                        );
                        break;
                      case "date":
                        cmp =
                          new Date(a.day.date).getTime() -
                          new Date(b.day.date).getTime();
                        break;
                      case "regular":
                        cmp =
                          (a.day.regularMinutes || 0) -
                          (b.day.regularMinutes || 0);
                        break;
                      case "overtime": {
                        const aOT = (a.day.overtimeEntries || [])
                          .filter((o) => !o.isAutoGenerated)
                          .reduce((s, o) => s + (o.requestedMinutes || 0), 0);
                        const bOT = (b.day.overtimeEntries || [])
                          .filter((o) => !o.isAutoGenerated)
                          .reduce((s, o) => s + (o.requestedMinutes || 0), 0);
                        cmp = aOT - bOT;
                        break;
                      ...
                      case "status":
                        cmp = (a.day.status || "").localeCompare(
                          b.day.status || "",
                        );
                        break;
                      default:
