...
const LeavePolicy = () => {
  ...
  const handleSavePolicy = async () => {
    ...
    try {
      const response = await leavePolicyService.updateClientPolicy(selectedClient.id, policyForm);
      if (response.success) {
  ...
  const fetchEmployeeBalances = useCallback(async () => {
    ...
    try {
      const response = await leavePolicyService.getEmployeeBalances({
        clientId: balanceClientFilter !== 'all' ? balanceClientFilter : undefined,
  ...
  const submitAdjustment = async () => {
    ...
    setAdjusting(true);
    try {
      const response = await leavePolicyService.adjustEmployeeBalance(selectedEmployee.id, {
  ...
  const fetchLeaveRequests = useCallback(async () => {
    ...
    try {
      const response = await leavePolicyService.getAllLeaveRequests({
        ...
        clientId: approvalClientFilter !== 'all' ? approvalClientFilter : undefined,
  ...
  const handleBulkApprove = async () => {
    ...
    setProcessingApproval(true);
    try {
      const response = await leavePolicyService.bulkApproveLeaveRequests(selectedRequests);
      if (response.success) {
        setSelectedRequests([]);
        fetchLeaveRequests();
      } else {
  ...
  const handleRunAccrual = async () => {
    if (!confirm('Run monthly accrual calculation for all employees with ACCRUED leave policy?')) {
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        alert(`Accrual complete! Processed: ${response.data.processed}, Updated: ${response.data.updated}`);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Configure leave policies, manage balances, and approve requests</p>
        </div>
        <Button
          variant="secondary"
          icon={RefreshCw}
                ...
                <TableBody>
                  {clients.map((client) => (
                    <TableRow key={client.id}>
                      ...
                      <TableCell>
                        {client.policy ? getEntitlementLabel(client.policy.paidLeaveEntitlementType) : '-'}
                      </TableCell>
                      <TableCell>
                    ...
                    <TableRow key={`${emp.id}-${emp.clientId}`}>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <Avatar name={emp.name} src={emp.profilePhoto} size="sm" />
                      ...
                      <TableCell>
                        {emp.balance.pending > 0 ? (
                          <span className="text-yellow-600">{emp.balance.pending} days</span>
                        ) : (
                          '0 days'
                        )}
                      </TableCell>
                      <TableCell>
                        <span className={`font-medium ${emp.balance.available < 0 ? 'text-red-600' : emp.balance.available < 3 ? 'text-yellow-600' : 'text-green-600'}`}>
            ...
            <Card padding="md">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold">{approvalStats.clientApproved || 0}</p>
                </div>
              </div>
            </Card>
            <Card padding="md">
                ...
                <TableHead>
                  <TableRow>
                    <TableHeader>
                      <input
                        type="checkbox"
                        checked={selectedRequests.length === leaveRequests.filter(r => r.status !== 'APPROVED' && r.status !== 'REJECTED').length}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setSelectedRequests(leaveRequests.filter(r => r.status !== 'APPROVED' && r.status !== 'REJECTED').map(r => r.id));
                          } else {
                            setSelectedRequests([]);
                          }
                        }}
                ...
                <TableBody>
                  {leaveRequests.map((request) => (
                    <TableRow key={request.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <Avatar name={request.employee.name} src={request.employee.profilePhoto} size="sm" />
                          <div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setShowPolicyModal(false)} disabled={savingPolicy}>
              Cancel
            </Button>
            <Button variant="primary" onClick={handleSavePolicy} disabled={savingPolicy}>
      ...
      >
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <h4 className="font-medium text-gray-900 border-b pb-2">Paid Leave Settings</h4>

            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={policyForm.allowPaidLeave || false}
                onChange={(e) => setPolicyForm({ ...policyForm, allowPaidLeave: e.target.checked })}
                className="rounded border-gray-300"
              />
                ...
                {policyForm.paidLeaveEntitlementType === 'FIXED' && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setPolicyForm({ ...policyForm, annualPaidLeaveDays: parseInt(e.target.value, 10) || 0 })}
                      className="input w-full"
                      min="0"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setPolicyForm({ ...policyForm, accrualRatePerMonth: parseFloat(e.target.value) || null })}
                        className="input w-full"
                        placeholder="e.g., 1.25"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Max Annual Days</label>
                      <input
                        type="number"
                        value={policyForm.annualPaidLeaveDays || 0}
                        onChange={(e) => setPolicyForm({ ...policyForm, annualPaidLeaveDays: parseInt(e.target.value, 10) || 0 })}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setPolicyForm({ ...policyForm, milestoneYearsRequired: parseInt(e.target.value, 10) || null })}
                        className="input w-full"
                        min="1"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Bonus Days After Milestone</label>
                      <input
                        type="number"
                        value={policyForm.milestoneBonusDays || ''}
                        onChange={(e) => setPolicyForm({ ...policyForm, milestoneBonusDays: parseInt(e.target.value, 10) || null })}
                        className="input w-full"
                        min="0"
                      />
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setPolicyForm({ ...policyForm, maxCarryoverDays: parseInt(e.target.value, 10) || 0 })}
                      className="input w-full"
                      min="0"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setPolicyForm({ ...policyForm, carryoverExpiryMonths: parseInt(e.target.value, 10) || null })}
                      className="input w-full"
                      placeholder="e.g., 3"
                    />
                  </div>
          ...
          <div className="space-y-4">
            <h4 className="font-medium text-gray-900 border-b pb-2">Unpaid Leave Settings</h4>

            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={policyForm.allowUnpaidLeave !== false}
                onChange={(e) => setPolicyForm({ ...policyForm, allowUnpaidLeave: e.target.checked })}
                className="rounded border-gray-300"
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setPolicyForm({ ...policyForm, requireTwoWeeksNotice: e.target.checked })}
                className="rounded border-gray-300"
              />
          ...
          {/* Overtime Section */}
          <div className="space-y-4">
            <h4 className="font-medium text-gray-900 border-b pb-2">Overtime Settings</h4>

            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={policyForm.allowOvertime !== false}
                onChange={(e) => setPolicyForm({ ...policyForm, allowOvertime: e.target.checked })}
                className="rounded border-gray-300"
              />
              ...
              <>
                <label className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => setPolicyForm({ ...policyForm, overtimeRequiresApproval: e.target.checked })}
                    className="rounded border-gray-300"
                  />
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Overtime Threshold (weekly hours)</label>
                  <input
                    type="number"
                    value={policyForm.overtimeThreshold || 40}
                    onChange={(e) => setPolicyForm({ ...policyForm, overtimeThreshold: parseInt(e.target.value, 10) || 40 })}
                    className="input w-full"
                    min="0"
                  />
                </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setShowAdjustModal(false)} disabled={adjusting}>
              Cancel
            </Button>
            <Button variant="primary" onClick={submitAdjustment} disabled={adjusting}>
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-gray-50 rounded-lg space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{selectedEmployee.clientName}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{selectedEmployee.balance.entitled} days</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Available</span>
                <span className={`font-medium ${selectedEmployee.balance.available < 0 ? 'text-red-600' : 'text-green-600'}`}>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setShowRejectModal(false)} disabled={processingApproval}>
              Cancel
            </Button>
            <Button variant="danger" onClick={submitReject} disabled={processingApproval}>
