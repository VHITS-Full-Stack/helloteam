...
export default function LunchBreakReview() {
  ...
  const [adjusting, setAdjusting] = useState(null); // breakId being adjusted
  ...
  const fetchBreaks = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await api.get(`/admin-portal/lunch-breaks/review?date=${date}`);
      if (res.success) {
        setBreaks(res.data);
        setTotal(res.total);
      } else {
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Lunch breaks that were auto-closed because the employee never pressed End Lunch Break</p>
        </div>
