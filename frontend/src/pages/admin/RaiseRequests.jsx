...
import { TrendingUp, Gift, Loader2, Check, X, Filter, ChevronDown, Upload, FileText, AlertCircle, Plus, ArrowRight } from "lucide-react";
...
const RaiseRequests = () => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (selectedRequest.type === "RAISE") {
        ...
        const autoNewPayRate = (selectedRequest.currentPayRate ?? 0) + raiseAmount;
  ...
  const handleCancelPendingBonus = async () => {
    if (pendingBonusId) {
      try { await adminPortalService.rejectRaiseRequest(pendingBonusId, "Cancelled before confirmation"); } catch (_) { /* ignore */ }
    }
    setGiveBonusStep(1);
    setPendingBonusId(null);
    setGiveBonusError("");
  ...
  const uniqueCandidateEmployees = [...new Map(
    raiseCandidates.map((c) => [c.employeeId, { id: c.employeeId, name: c.employeeName, payRate: c.currentPayRate }])
  ).entries()].map(([, v]) => v);
  ...
  const selectedCandidate = raiseCandidates.find(
    (c) => c.employeeId === giveRaiseForm.clientId
      ? false
      : c.employeeId === giveRaiseForm.employeeId && c.clientId === giveRaiseForm.clientId
  ) || raiseCandidates.find(
    (c) => c.employeeId === giveRaiseForm.employeeId && c.clientId === giveRaiseForm.clientId
  );

  const raiseAmt = parseFloat(giveRaiseForm.employeeRaiseAmount) || 0;
  const coveredAmt = giveRaiseForm.coverageType === "FULL"
    ? raiseAmt
  ...
  const handleGiveRaiseSubmit = async () => {
    const { employeeId, clientId, coverageType, employeeRaiseAmount, clientCoveredAmount, effectiveDate } = giveRaiseForm;
    if (!employeeId || !clientId) { setGiveRaiseError("Please select an employee and client."); return; }
    if (!employeeRaiseAmount || parseFloat(employeeRaiseAmount) <= 0) { setGiveRaiseError("Enter a valid raise amount."); return; }
    if (coverageType === "PARTIAL") {
      const covered = parseFloat(clientCoveredAmount);
      if (!covered || covered <= 0 || covered >= parseFloat(employeeRaiseAmount)) {
        setGiveRaiseError("Client covered amount must be between $0 and the employee raise amount.");
        return;
      }
    }
    if (!effectiveDate) { setGiveRaiseError("Please select an effective date."); return; }
    ...
    try {
      const res = await adminPortalService.giveRaise({
        ...
        clientCoveredAmount: coverageType === "FULL"
  ...
  const handleCancelPendingRaise = async () => {
    if (pendingRaiseId) {
      try {
        await adminPortalService.rejectRaiseRequest(pendingRaiseId, "Cancelled before confirmation");
      } catch (_) { /* ignore */ }
    }
    setGiveRaiseStep(1);
    setPendingRaiseId(null);
  ...
  const getCoverageBadge = (coverageType) => {
    ...
    return <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${map[coverageType] || ""}`}>{coverageType}</span>;
  };
          ...
          <tr key={rr.id} className="border-b border-gray-50 last:border-b-0 hover:bg-gray-50/50">
            <td className="py-3 px-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900 whitespace-nowrap">{rr.employee.firstName} {rr.employee.lastName}</p>
                  {rr.raisedBy === "ADMIN" && <span className="text-[10px] text-primary-600 font-medium">Admin raise</span>}
                </div>
              </div>
            </td>
            <td className="py-3 px-3 text-sm text-gray-700 whitespace-nowrap">{rr.client.companyName}</td>
            <td className="py-3 px-3 text-center">{getCoverageBadge(rr.coverageType)}</td>
            <td className="py-3 px-3 text-center text-sm font-semibold text-green-700">
              {rr.employeeRaiseAmount != null ? `+${rr.employeeRaiseAmount.toFixed(2)}` : "—"}
            </td>
          ...
          <button
            onClick={() => {
              ...
              setGiveBonusForm((prev) => ({
                ...
                effectiveDate: rr.effectiveDate ? rr.effectiveDate.split("T")[0] : new Date().toISOString().split("T")[0],
                reason: rr.reason || "",
              }));
              setShowGiveBonusModal(true);
            }}
  ...
  const CoverageInfo = ({ type, raiseAmt, coveredAmt, currentPay, currentBill }) => {
    if (!type) return null;
    ...
    const descriptions = {
      ...
      PARTIAL: "Client covers part of the raise. Employee pay increases fully; client billing increases partially.",
        ...
        {raiseAmt > 0 && (
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="space-y-1">
              <p className="text-gray-400 uppercase tracking-wide text-[10px] font-semibold">Employee Pay Rate</p>
              <p className="text-gray-500">${(currentPay || 0).toFixed(2)} → <span className="font-semibold text-blue-700">${newPay.toFixed(2)}</span></p>
            </div>
            <div className="space-y-1">
              <p className="text-gray-400 uppercase tracking-wide text-[10px] font-semibold">Client Bill Rate</p>
              {type === "NONE"
                ? <p className="text-gray-500">No change (<span className="font-semibold">${(currentBill || 0).toFixed(2)}</span>)</p>
                : <p className="text-gray-500">${(currentBill || 0).toFixed(2)} → <span className="font-semibold text-primary-700">${newBill.toFixed(2)}</span></p>
              }
            </div>
          </div>
        )}
  ...
  return (
    <div className="space-y-5 animate-fade-in">
          ...
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            {giveBonusStep === 1 && (
              <>
                        ...
                        {[
                          ...
                          <button
                            ...
                            className={`p-2.5 rounded-lg border-2 text-left transition-colors ${
                              giveBonusForm.coverageType === opt.key ? "border-amber-500 bg-amber-50" : "border-gray-200 hover:border-gray-300"
                            }`}
                          >
                ...
                <div className="flex justify-end gap-3 mt-5">
                  ...
                  >
                    {giveBonusLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <ArrowRight className="w-4 h-4" />}
                    Review Bonus
                  </button>
                </div>
              </>
            ...
            {giveBonusStep === 2 && (() => {
              const bonusCandidate = raiseCandidates.find(
                (c) => c.employeeId === giveBonusForm.employeeId && c.clientId === giveBonusForm.clientId
              );
              const bonusAmt = parseFloat(giveBonusForm.amount) || 0;
              const clientCovered = giveBonusForm.coverageType === "FULL" ? bonusAmt
                : giveBonusForm.coverageType === "NONE" ? 0
              ...
              return (
                <>
                  ...
                  <p className="text-sm text-gray-500 mb-4">Review the details before applying this bonus.</p>

                  <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 space-y-3 mb-4 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-500">Employee</span>
                      <span className="font-medium text-gray-900">{bonusCandidate?.employeeName || giveBonusForm.employeeId}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Client</span>
                      <span className="font-medium text-gray-900">{bonusCandidate?.clientName || giveBonusForm.clientId}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Bonus Amount</span>
                      <span className="font-semibold text-amber-700">${bonusAmt.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    {giveBonusForm.coverageType === "PARTIAL" && (
                      <>
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-gray-900">${clientCovered.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Admin absorbs</span>
                          <span className="font-medium text-gray-900">${adminAbsorbed.toFixed(2)}</span>
                        </div>
                      </>
                    )}
                    <div className="flex justify-between">
                      <span className="text-gray-500">Effective Date</span>
                      <span className="text-gray-900">{new Date(giveBonusForm.effectiveDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
                    </div>
                    {giveBonusForm.reason && (
                      <div className="flex justify-between gap-4">
                        <span className="text-gray-500 shrink-0">Reason</span>
                        <span className="text-gray-700 text-right">{giveBonusForm.reason}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex justify-end gap-3">
                    <button onClick={handleCancelPendingBonus} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Back</button>
                    <button
      ...
      {/* ── Give a Raise Modal ───────────────────────────────────────────── */}
      {showGiveRaiseModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm" onClick={() => setShowGiveRaiseModal(false)}>
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>

            {/* Step indicator */}
            <div className="flex items-center gap-2 mb-5">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${giveRaiseStep >= 1 ? "bg-primary text-white" : "bg-gray-100 text-gray-400"}`}>1</div>
              <div className={`flex-1 h-0.5 ${giveRaiseStep >= 2 ? "bg-primary" : "bg-gray-200"}`} />
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${giveRaiseStep >= 2 ? "bg-primary text-white" : "bg-gray-100 text-gray-400"}`}>2</div>
            </div>

            {/* ── Step 1: Form ── */}
            {giveRaiseStep === 1 && (
              <>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Give a Raise</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => handleGiveRaiseFormChange("employeeId", e.target.value)}
                        >
                          <option value="">Select employee...</option>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => handleGiveRaiseFormChange("clientId", e.target.value)}
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          <button
                            key={opt.key}
                            type="button"
                            onClick={() => handleGiveRaiseFormChange("coverageType", opt.key)}
                            className={`p-2.5 rounded-lg border-2 text-left transition-colors ${
                          ...
                          >
                            <p className={`text-sm font-semibold ${giveRaiseForm.coverageType === opt.key ? "text-primary-700" : "text-gray-700"}`}>{opt.label}</p>
                            <p className="text-[10px] text-gray-500 mt-0.5">{opt.desc}</p>
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => handleGiveRaiseFormChange("employeeRaiseAmount", e.target.value)}
                      ...
                      {giveRaiseForm.coverageType === "PARTIAL" && (
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => handleGiveRaiseFormChange("clientCoveredAmount", e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleGiveRaiseFormChange("effectiveDate", e.target.value)}
                        className="input w-full"
                      />
                    </div>
                ...
                <div className="flex justify-end gap-3 mt-5">
                  <button onClick={() => setShowGiveRaiseModal(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Cancel</button>
                  <button
                  ...
                  >
                    {giveRaiseLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <ArrowRight className="w-4 h-4" />}
                    Review
                  </button>
                </div>
              </>
            ...
            {giveRaiseStep === 2 && (
              <>
                ...
                <p className="text-sm text-gray-500 mb-4">Review the details below before applying the raise.</p>

                {(() => {
                  const candidate = selectedCandidate || raiseCandidates.find(
                    (c) => c.employeeId === giveRaiseForm.employeeId && c.clientId === giveRaiseForm.clientId
                  );
                  const empName = candidate?.employeeName || selectedRequest?.employee
                    ? `${selectedRequest?.employee?.firstName} ${selectedRequest?.employee?.lastName}`
                    : "—";
                  const clientName = candidate?.clientName || selectedRequest?.client?.companyName || "—";
                  const coverage = giveRaiseForm.coverageType || selectedRequest?.coverageType || "—";
                  const raiseAmtVal = parseFloat(giveRaiseForm.employeeRaiseAmount) || selectedRequest?.employeeRaiseAmount || 0;
                  const coveredAmtVal = coverage === "FULL" ? raiseAmtVal
                    : coverage === "NONE" ? 0
                    : parseFloat(giveRaiseForm.clientCoveredAmount) || selectedRequest?.clientCoveredAmount || 0;
                  const currentPay = candidate?.currentPayRate ?? selectedRequest?.currentPayRate ?? 0;
                  const currentBill = candidate?.currentBillRate ?? selectedRequest?.currentBillRate ?? 0;
                  const effDate = giveRaiseForm.effectiveDate || selectedRequest?.effectiveDate?.split("T")[0] || "—";
                    ...
                    <div className="space-y-3">
                      <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 space-y-3">
                        ...
                        <div className="border-t border-gray-200 pt-3 grid grid-cols-2 gap-3 text-sm">
                          <div>
                            <p className="text-gray-400 text-xs mb-1">Pay Rate</p>
                            <p className="text-gray-500">${currentPay.toFixed(2)} → <span className="font-bold text-blue-700">${(currentPay + raiseAmtVal).toFixed(2)}</span></p>
                            <p className="text-xs text-green-600 mt-0.5">+${raiseAmtVal.toFixed(2)}/hr</p>
                          </div>
                          <div>
                            <p className="text-gray-400 text-xs mb-1">Bill Rate</p>
                            {coverage === "NONE" ? (
                              <p className="text-gray-500">No change</p>
                            ) : (
                              <>
                                <p className="text-gray-500">${currentBill.toFixed(2)} → <span className="font-bold text-primary-700">${(currentBill + coveredAmtVal).toFixed(2)}</span></p>
                                <p className="text-xs text-green-600 mt-0.5">+${coveredAmtVal.toFixed(2)}/hr</p>
                              </>
                            )}
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-sm border-t border-gray-200 pt-3">
                          <span className="text-gray-500">Effective Date</span>
                          <span className="font-medium text-gray-900">{effDate}</span>
                        </div>
                        {coverage !== "NONE" && (
                        ...
                        {coverage === "NONE" && (
                          <div className="flex items-start gap-1.5 bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-xs text-gray-600">
                            <AlertCircle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
                            Client will not be notified (billing rate unchanged).
                          </div>
                        )}
                      </div>
                ...
                <div className="flex justify-between gap-3 mt-5">
                  ...
                  <button
                    ...
                    className="px-4 py-2 text-sm font-medium text-white bg-primary rounded-lg hover:bg-primary-dark disabled:opacity-50 transition-colors inline-flex items-center gap-2"
                  >
                    {giveRaiseLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                    Apply Raise
                  </button>
                </div>
              </>
      ...
      {showApproveModal && selectedRequest && (() => {
        ...
        const autoNewPayRate = (selectedRequest.currentPayRate ?? 0) + raiseAmount;
        const autoNewBillRate = (selectedRequest.currentBillRate ?? 0) + raiseAmount;
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm" onClick={() => setShowApproveModal(false)}>
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="text-sm text-gray-500 mb-4">
                {isRaise ? (
                  <>Approve pay raise of <span className="font-semibold text-green-700">+${raiseAmount.toFixed(2)}/hr</span> for <span className="font-semibold">{selectedRequest.employee.firstName} {selectedRequest.employee.lastName}</span> from {selectedRequest.client.companyName}?</>
                ) : (
                  <>Approve <span className="font-semibold text-amber-700">${selectedRequest.amount?.toFixed(2)}</span> bonus for <span className="font-semibold">{selectedRequest.employee.firstName} {selectedRequest.employee.lastName}</span> from {selectedRequest.client.companyName}?</>
                )}
              </p>
              ...
              {isRaise && (
                <div className="mb-4 bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-2.5 text-sm border-b border-gray-200">
                    <span className="text-gray-500">Current employee pay rate</span>
                    <span className="font-medium text-gray-700">${(selectedRequest.currentPayRate ?? 0).toFixed(2)}/hr</span>
                  </div>
                  <div className="flex items-center justify-between px-4 py-2.5 text-sm border-b border-gray-200">
                    <span className="text-gray-500">Raise amount</span>
                    <span className="font-medium text-green-700">+${raiseAmount.toFixed(2)}/hr</span>
                  </div>
                  <div className="flex items-center justify-between px-4 py-2.5 text-sm bg-blue-50 border-b border-blue-100">
                    <span className="font-semibold text-gray-700">New employee pay rate</span>
                    <span className="font-bold text-blue-700">${autoNewPayRate.toFixed(2)}/hr</span>
                  </div>
                  <div className="flex items-center justify-between px-4 py-2.5 text-sm border-b border-gray-200">
                    <span className="text-gray-500">Current client billing rate</span>
                    <span className="font-medium text-gray-700">${(selectedRequest.currentBillRate ?? 0).toFixed(2)}/hr</span>
                  </div>
                  <div className="flex items-center justify-between px-4 py-2.5 text-sm bg-primary-50">
                    <span className="font-semibold text-gray-700">New client billing rate</span>
                    <span className="font-bold text-primary-700">${autoNewBillRate.toFixed(2)}/hr</span>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-3 border-t border-gray-100 pt-4">
                <p className="text-sm font-medium text-gray-700">Confirm employee was notified <span className="text-red-500">*</span></p>
                <div
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <input type="radio" name="confirmationType" checked={confirmationType === "proof"} onChange={() => { setConfirmationType("proof"); setApproveError(""); }} className="accent-green-600" />
                    <span className="text-sm font-medium text-gray-800">Upload proof of employee notification</span>
                  </div>
                  {confirmationType === "proof" && (
                    <div className="mt-2 ml-5">
                      <input ref={proofInputRef} type="file" accept="image/jpeg,image/png,image/webp,application/pdf" className="hidden" onChange={(e) => { setProofFile(e.target.files[0] || null); setApproveError(""); }} />
                      {proofFile ? (
                        <div className="flex items-center gap-2 px-3 py-2 bg-white border border-green-200 rounded-lg">
                          ...
                          <button onClick={(e) => { e.stopPropagation(); setProofFile(null); }} className="text-gray-400 hover:text-gray-600"><X className="w-4 h-4" /></button>
                        </div>
                      ) : (
                        <button type="button" onClick={(e) => { e.stopPropagation(); proofInputRef.current?.click(); }} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm text-gray-600 border border-dashed border-gray-300 rounded-lg hover:border-green-400 hover:bg-white transition-colors">
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <input type="radio" name="confirmationType" checked={confirmationType === "note"} onChange={() => { setConfirmationType("note"); setApproveError(""); }} className="accent-green-600" />
                  ...
                  {confirmationType === "note" && (
                    <div className="mt-2 ml-5" onClick={(e) => e.stopPropagation()}>
                      ...
                      />
                      <p className={`text-xs mt-1 text-right ${approvalNote.length >= 500 ? "text-red-500" : "text-gray-400"}`}>{approvalNote.length}/500</p>
                    </div>
                  )}
                </div>
                {approveError && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm" onClick={() => setShowRejectModal(false)}>
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">
              {selectedRequest.raisedBy === "ADMIN" ? "Cancel" : "Reject"} {selectedRequest.type === "BONUS" ? "Bonus" : "Raise"} Request
            </h3>
            <p className="text-sm text-gray-500 mb-4">
              {selectedRequest.raisedBy === "ADMIN" ? "Cancel" : "Reject"} for <span className="font-semibold">{selectedRequest.employee.firstName} {selectedRequest.employee.lastName}</span> from {selectedRequest.client.companyName}.
            </p>
            <textarea
              value={adminNotes}
            ...
            <div className="flex justify-end gap-3 mt-4">
              ...
              >
                {actionLoading === selectedRequest.id && <Loader2 className="w-4 h-4 animate-spin" />}
                {selectedRequest.raisedBy === "ADMIN" ? "Cancel Raise" : "Reject"}
              </button>
            </div>
          </div>
        </div>
